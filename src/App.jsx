...
export default function App() {
  ...
  const topBarContainerStyle = {
    position: 'absolute', top: '4vh', left: '4vw', right: '4vw', display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 10, pointerEvents: 'none' 
  };
  ...
  const calculateResults = () => {
    ...
    const dimensionsScore = {
      Business: (answers.Q1 || 0) + (answers.Q2 || 0), People: answers.Q3 || 0, Governance: (answers.Q4 || 0) + (answers.Q5 || 0),
      Platform: (answers.Q6 || 0) + (answers.Q7 || 0), Security: (answers.Q8 || 0) + (answers.Q9 || 0), Operations: answers.Q10 || 0,
    };
    let levelKey = totalPercentage <= 40 ? "exploring" : totalPercentage <= 60 ? "adopting" : totalPercentage <= 80 ? "implementing" : "transforming";
    return { totalPoints, totalPercentage, dimensionsScore, levelData: t(`res.${levelKey}`, { returnObjects: true }) };
  };

  const floatingControls = (
    <div className="no-print" style={{ position: 'fixed', bottom: '4vh', right: '4vw', zIndex: 9999, display: 'flex', gap: '12px', alignItems: 'center', background: '#ffffff', padding: '8px 12px', borderRadius: '50px', boxShadow: '0 10px 30px rgba(0,0,0,0.15)', border: '1px solid #e2e8f0' }}>
      {hasStarted && (
    ...
    return (
      <div className="results-page-wrapper" style={{ minHeight: '100vh', width: '100vw', ...lightFuturisticBackgroundStyle, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '14vh 3vw 5vh 3vw', boxSizing: 'border-box', position: 'relative' }}>
        ...
        <div className="results-main-card" style={{ position: 'relative', zIndex: 1, maxWidth: '1400px', width: '100%', minHeight: '85vh', height: 'auto', background: '#ffffff', borderRadius: '24px', boxShadow: '0 30px 70px rgba(0,0,0,0.5)', boxSizing: 'border-box', overflow: 'visible', display: 'flex', flexDirection: 'column' }}>
          ...
          {/* TÍTULO PDF (Se oculta en Web) */}
          <div className="print-only-block print-title-container" style={{ padding: '2.5vh 4vw', borderBottom: '1px solid #eee', textAlign: 'center' }}>
             <h2 style={{ fontSize: '1.8rem', color: oneDataDarkBlue, fontWeight: '900', margin: 0 }}>{t('dashTitle')}</h2>
          </div>

          <div className="print-content-padding" style={{ padding: '3vh 4vw', display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
            ...
            <div className="results-main">
              ...
              <div className="dimensions-grid">
                {Object.entries(results.dimensionsScore).map(([dimension, score]) => {
                  ...
                  return (
                    <div key={dimension} className="dimension-card print-avoid-break">
  ...
  return (
    <div className="results-page-wrapper" style={{ minHeight: '100vh', width: '100vw', ...lightFuturisticBackgroundStyle, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '14vh 2vw 5vh 2vw', boxSizing: 'border-box', position: 'relative' }}>
      {floatingControls}
      <div style={topBarContainerStyle}>
        <img src={clusterLogo} alt="Cluster" style={{ height: 'clamp(40px, 6vh, 65px)', objectFit: 'contain', transform: 'scale(1.8)', marginLeft: '2vw' }} />
        <img src={onedataWhite} alt="OneData" style={{ height: 'clamp(35px, 5.5vh, 60px)', objectFit: 'contain' }} />
